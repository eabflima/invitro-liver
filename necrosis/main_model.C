...
void RungeKutta(std::vector<double>& QoI,const double time_step,vector<double> Parameters,const double nut_mean);
void fmodel(std::vector<double> qoi,std::vector<double>& auxiliar,vector<double> Parameters,const double nut_mean);

void main_code(vector<double>& TUMOR_MASS,vector<double> Parameters,double tumor_mean,const double nut_mean,vector<double> SaveTime){
  ...
  const unsigned int number_equations = 2;
  std::vector<double> QoI(number_equations,0.0);
  
  QoI[0] = tumor_mean;
  QoI[1] = 0.0;
  TUMOR_MASS.push_back(QoI[0]-QoI[1]);
  /////----- Defining parameters -----/////
  ...
  do
    {
      /////----- Prepare time step-----/////
      t_step++;
      RungeKutta(QoI,time_step,Parameters,nut_mean);
...
void RungeKutta(std::vector<double>& QoI,const double time_step,vector<double> Parameters,const double nut_mean){
  ...
  fmodel(qoi,auxiliar,Parameters,nut_mean);
...
void fmodel(std::vector<double> qoi,std::vector<double>& auxiliar,vector<double> Parameters,const double nut_mean){
  ...
  double nec  = Parameters[3];
  auxiliar[0]=prol*qoi[0]*nut_mean*(1.0-qoi[0]/supp)-apop*qoi[0];
  if(nut_mean<1.0)
    auxiliar[1]=nec*(qoi[0]-qoi[1]);
}
